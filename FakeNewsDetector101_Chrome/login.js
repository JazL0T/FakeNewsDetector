...
async function handleSubmit(isRegister = false) {
  ...
  try {
    ...
    if (isRegister) {
      ...
      setTimeout(() => {
        // Open success page in new tab (allows window.close to work there)
    ...
    backPopup.style.display = "none"; // Hide back button once logged in

    setTimeout(() => {
      // Open welcome page in new tab (so Start Scanning button can close it)
...
passwordInput.addEventListener("input", () => {
  ...
  if (pass.length === 0) {
    ...
    passwordHint.style.color = "#e53935";
  } else {
